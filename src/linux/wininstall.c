...
// ---------------------------------------------------------------------------
typedef struct { int bypass, nro, privacy; } Wue;
...
static int wue_parse(const char *spec, Wue *w, char *err, unsigned long cap) {
  ...
  if (spec && spec[0]) {
    ...
    w->bypass = !!strstr(tmp, ",bypass,") || !!strstr(tmp, ",all,");
    w->nro = !!strstr(tmp, ",nro,") || !!strstr(tmp, ",all,");
    w->privacy = !!strstr(tmp, ",privacy,") || !!strstr(tmp, ",all,");
    if (!w->bypass && !w->nro && !w->privacy) {
      snprintf(err, cap, "unknown --wue item (bypass,nro,privacy,all)");
      return -1;
    }
  } else {
    w->bypass = 1; // default: hardware requirement bypass only
  }
  return 0;
}
...
static int write_xml(const char *path, const char *arch, int winpe_bypass, int nro,
                     int privacy, char *err, unsigned long cap) {
  FILE *f = fopen(path, "w");
  if (!f) { snprintf(err, cap, "cannot write '%s': %s", path, strerror(errno)); return -1; }
  char ns[512], comp[512];
  ...
  snprintf(comp, sizeof comp, "processorArchitecture=\"%s\" publicKeyToken=\"31bf3856ad364e35\" "
           "language=\"neutral\" versionScope=\"nonSxS\"%s", arch, ns);
  ...
  if (privacy)
    fprintf(f, "  <settings pass=\"oobeSystem\">\n"
               "    <component name=\"Microsoft-Windows-Shell-Setup\" %s>\n"
               "      <OOBE>\n        <HideEULAPage>true</HideEULAPage>\n"
               "        <HideWirelessSetupInOOBE>true</HideWirelessSetupInOOBE>\n"
               "        <ProtectYourPC>3</ProtectYourPC>\n      </OOBE>\n    </component>\n"
               "    <component name=\"Microsoft-Windows-SecureStartup-FilterDriver\" %s>\n"
...
int rufux_write_unattend(const char *dir, const char *wue, char *err, unsigned long cap) {
  ...
  return write_xml(path, media_arch(dir), w.bypass, w.nro, w.privacy, err, cap);
}
...
int rufux_windows_customize(const char *root, const char *wue, const char *drivers,
                            RufuxWueLog log, void *lu, char *err, unsigned long cap) {
  Wue w;
  ...
  if (fallback || w.nro || w.privacy) {
    ...
    if (write_xml(path, arch, fallback, w.nro, w.privacy, err, cap) != 0) return -1;
